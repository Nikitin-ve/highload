...
3. Give a proper name to your service and replace all the occurrences of "highload" string with that name
   (could be done via `find . -not -path "./third_party/*" -not -path ".git/*" -not -path './build-*' -type f | xargs sed -i 's/highload/YOUR_SERVICE_NAME/g'`).
